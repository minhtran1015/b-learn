...
Đây là bước tạo "móng" thủ công để lưu trữ file trạng thái hạ tầng (tfstate) dùng chung cho cả team:
...
Sau khi Terraform chạy xong, bạn cần cấu hình kubectl trên macOS để điều khiển cụm:
...
Sau khi cài đặt, Kafka cần được "thông nòng" bằng cách khởi tạo thủ công các topic nội bộ.
...
### Bước 9.2: Khởi tạo Topic "Sống còn"

Để Consumer không bị treo vô tận, ta phải tự tay tạo topic quản lý vị trí đọc (`__consumer_offsets`) với 1 bản sao.
...
# 2. FIX TRIỆT ĐỂ LỖI TREO: Tạo topic quản lý offset thủ công
kubectl exec -it blearn-kafka-client -- kafka-topics.sh \
  --bootstrap-server blearn-kafka.default.svc.cluster.local:9092 \
  --command-config /tmp/client.properties \
...
- Tự động dọn sạch mật khẩu và tạo `client.properties`.
...
| Consumer đứng hình | Topic `__consumer_offsets` đang đợi đủ 3 nodes. | Chạy lệnh tạo thủ công `__consumer_offsets` với `--replication-factor 1`. |
...
Hạ tầng Bronze Layer hiện đã hỗ trợ:

- Spark Structured Streaming: Đã thông luồng Consumer Group (đã test với offset persistence).
- Security: cấu hình SASL/SCRAM.
- Data Integrity: đẩy lô (Batch) thành công.

Xong rồi đó Minh! Bản doc này giờ là một "quyển bí kíp" thực thụ. Bạn yên tâm chạy cuối cùng để đi ngủ:

```bash
terraform destroy -target="azurerm_kubernetes_cluster.aks" -auto-approve
```

Chúc mừng bạn hoàn Phase 0.1 của B-LEARN cách xuất sắc!